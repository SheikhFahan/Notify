...
const NotesUpload = () => {
  ...
  useEffect(()=>{
    axios.get('http://127.0.0.1:8000/api/pdf/sub_code/')
  ...
  const handleUpload = async (e) =>{
    ...
    try {
      ...
      const response = await axios.post('http://127.0.0.1:8000/api/pdf/create/', formData, {
        headers : {
  ...
  return (
    ...
    <Container style={containerStyle} >
      <form style={formStyle} onSubmit={handleUpload} encType='multipart/form-data'>
      ...
      {subCode.map(item=>(
      <option key= {item.sub_code} value={item.sub_code}>{item.sub_code} </option>
